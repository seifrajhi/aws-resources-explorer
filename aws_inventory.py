...
def flatten_dict(d, prefix=''):
    ...
    for key, value in d.items():
        ...
        if isinstance(value, dict):
            items.extend(flatten_dict(value, new_prefix))
        elif isinstance(value, list):
            items.extend(flatten_list(value, new_prefix))
        else:
            items.append((new_prefix, value))
    return items

def flatten_list(l, prefix=''):
    items = []
    for i, item in enumerate(l):
        new_prefix = f"{prefix}[{i}]"
        if isinstance(item, dict):
            items.extend(flatten_dict(item, new_prefix))
        elif isinstance(item, list):
            items.extend(flatten_list(item, new_prefix))
        else:
            items.append((new_prefix, item))
    return items
...
if __name__ == "__main__":
    ...
    """with open('gui/aws_inventory_data-default.json') as f:
        data = json.load(f)

    flat_data = flatten_dict(data)
    max_key_length = max(len(key) for key, value in flat_data)*/
