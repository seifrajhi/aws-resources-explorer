...
class ApiInvoker(object):
    ...
    def __init__(self, script_args, svc_descriptors, ops_count):
        ...
        # Create a dictionary with only the required parameters

        # Create a dictionary with only the required parameters

        # self.credentials = session.get_credentials()
        self.credentials = botocore.session.Session().get_credentials()
        print(self.credentials)
        if self.credentials is None:
            print("Failed to retrieve AWS credentials.")

        # access_key and secret_key can be passed directly to a client constructor
